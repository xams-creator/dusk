import Dusk, { PluginFunction, inBrowser } from '../..';
        ...
        return {
            ...
            setup() {
                if (inBrowser()) {
                    ...
                    const onError = event => {
                        // todo fix 当直接 throw new Error 时,react-dom 会进行一次 rethrow ，导致2次onError
                        // 调用前 event.defaultPrevented === false ,调用后 event.defaultPrevented === true,是否可以做某事
                        app.emit(
                            APP_HOOKS_ON_ERROR,
                            String(event.message || event.error?.message || event.reason?.message),
                            event
                        );
